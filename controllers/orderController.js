...
import productModel from "../Models/productModel.js"; //:D
...
export const createOrderController = async (req, res) => {
  try {
    ...
    for (let i = 0; i < orderItems.length; i++) {
      ...
      product.stock -= orderItems[i].quantity; //removing
      await product.save();
    }
    res.status(201).send({
      success: true,
...
export const getMyOrdersController = async (req, res) => {
  try {
    // find orders on the base of user ID
...
export const paymentsController = async (req, res) => {
  try {
    ...
    //client_secret are payments
    const { client_secret } = await stripe.paymentIntents.create({
      amount: Number(totalAmount * 100), //string convert into No.
      currency: "usd", //
    });
    res.status(200).send({
      success: true,
